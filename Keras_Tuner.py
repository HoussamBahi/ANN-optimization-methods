...
from kerastuner.tuners import bayesian
...
def architecture(hyperparam):
    initializer = "glorot_uniform"
    model = tf.keras.Sequential()
    lrng = hyperparam.Choice('lr', values=[1e-2, 0.008, 0.006, 0.004, 0.002, 1e-3, 0.0005])
    for i in range(2):
        model.add(
    ...
    model.compile(loss="mean_squared_error", optimizer= tf.keras.optimizers.RMSprop(lr=lrng, rho=0.9, epsilon=None, decay=0.0), metrics=[ tf.keras.losses.MSE,"mean_absolute_percentage_error"])
    return model
...
EPOCHS = [1000]
# MODEL
# model1 = architecture(hyperparam)
#model1.summary()
# pretrained weights:
...
tuner_search = RandomSearch(architecture,
