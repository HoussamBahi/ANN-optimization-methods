...
def architecture( nb_laters=2, nb_neurons=50,init_mode='uniform'):
    initializer = "glorot_uniform"
    model = tf.keras.Sequential()

    for i in range(nb_laters):
        model.add(
            tf.keras.layers.Dense(nb_neurons, input_dim=11, kernel_initializer=init_mode, activation='softsign'))
